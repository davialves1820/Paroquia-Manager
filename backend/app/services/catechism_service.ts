...
export default class CatechismService {
  ...
  async updateStudent(
    studentId: number,
    data: { name?: string; hasBaptism?: boolean; hasFirstEucharist?: boolean; status?: string }
  ) {
  ...
  async getClassDetails(classId: number) {
    ...
    const studentsWithFrequency = await Promise.all(
      catechismClass.students.map(async (student) => {
  ...
  async calculateFrequency(classId: number, studentId: number) {
    const meetings = await CatechismMeeting.query().where('classId', classId)
    ...
    const presentAttendances = await Attendance.query()
      ...
      .whereIn(
        'date',
        meetings.map((m) => m.date.toISODate()!)
      )
      .count('* as total')
  ...
  async getMetrics(year?: number) {
    ...
    if (year) {
      totalStudentsQuery = totalStudentsQuery
        .innerJoin('catechism_classes', 'catechism_classes.id', 'catechism_students.class_id')
        .where('catechism_classes.year', year)
      completedStudentsQuery = completedStudentsQuery
        .innerJoin('catechism_classes', 'catechism_classes.id', 'catechism_students.class_id')
        .where('catechism_classes.year', year)
    }
